# from sklearn.neighbors import NearestNeighbors
from io import SEEK_CUR
from typing import Type
from numpy.core.defchararray import partition
from numpy.lib.type_check import real
from sklearn.neighbors import KDTree
# import sklearn.neighbors
import pandas as pd
import numpy as np
import math
import csv
from matplotlib import pyplot as plt
# from KBsomte.SMOTE.Density import RelativeDensity
# from experiment_utils import load_2classes_data
from collections import Counter



class Natural_Neighbor(object):

    def __init__(self):
        self.nan_edges = {}        # Graph of mutual neighbors
        self.nan_num = {}          # Number of natural neighbors of each instance
        self.target = []           # Set of classes
        self.data = []             # Instance set
        self.knn = {}              # Structure that stores the neighbors of each instance
        self.nan = {}           # Structure that stores the nan neighbors of each instance
        self.relative_cox = []
        

    def asserts(self):
        self.nan_edges = set()
        for j in range(len(self.data)):
            self.knn[j] = set()
            self.nan[j] = set()     # 初始化自然邻居
            self.nan_num[j] = 0

    # Returns the number of instances that have no natural neighbor
    def count(self):
        nan_zeros = 0
        for x in self.nan_num:
            if self.nan_num[x] == 0:
                nan_zeros += 1
        return nan_zeros

    # Returns the indices of the closest neighbors
    def findKNN(self, inst, r, tree):
        dist, ind = tree.query([inst], r+1)  # r+1最近邻的距离, 下标
        return np.delete(ind[0], 0)

    # 返回 NaNe
    def algorithm(self):
        # ASSERT
        tree = KDTree(self.data)    # 构建kD树
        self.asserts()              # 初始化计算自然邻居参数:self.knn,self.nan_num,self.nan_edges
        flag = 0
        r = 2   # 自然特征初始化为1
        cnt_before = -1  # 初始化当前邻居数为0的样本总数为负数

        while(flag == 0):
            for i in range(len(self.data)):
                knn = self.findKNN(self.data[i], r, tree)
                n = knn[-1]  # 获取r-最近邻
                self.knn[i].add(n)  # 添加i的r-最近邻n
                if(i in self.knn[n] and (i, n) not in self.nan_edges):  # 判断自然邻居
                    self.nan_edges.add((i, n))
                    self.nan_edges.add((n, i))
                    self.nan[i].add(n)      # 记录自然邻居对
                    self.nan[n].add(i)
                    self.nan_num[i] += 1    # i,n的自然邻居数加一
                    self.nan_num[n] += 1

            cnt_after = self.count()  # 获取当前邻居数为0的样本总数
            if cnt_after < math.sqrt(len(self.data)):
                flag = 1
            else:
                r += 1
            cnt_before = cnt_after
        return r


    # 相对密度
    def RelativeDensity(self, min_i, maj_i):
        """离群点密度-2,噪声点-3,多数类0"""
        self.relative_cox = [0]*len(self.target)
        for i, num in self.nan.items():   # num为i的自然邻居集合
            if self.target[i] == min_i:     #少数类
                if len(num) == 0 :  # 标记离群点样本 
                    self.relative_cox[i] = -2
                else:
                    absolute_min,min_num,absolute_max,maj_num = 0,0,0,0
                    maj_index = []

                    for j in iter(num): # 遍历自然邻居集合
                        if self.target[j] == min_i:
                            absolute_min += np.sqrt(np.sum(np.square(self.data[i]-self.data[j])))
                            min_num += 1
                        elif self.target[j] == maj_i:
                            absolute_max += np.sqrt(np.sum(np.square(self.data[i]-self.data[j])))
                            maj_num += 1
                            maj_index.append(j)
                    self.nan[i].difference_update(maj_index)    #去掉自然邻居中的多数类样本

                    if min_num == 0:    # 噪声点: 其自然邻居集合全是多数类,过滤少数类(删除噪声点)
                        self.relative_cox[i] = -3 
                    elif maj_num == 0 :         # 安全点: 少数类其自然邻居集合都为少数类
                        relative = min_num/absolute_min
                        self.relative_cox[i] = relative
                    else:           # 边界样本
                        relative = (min_num/absolute_min)/(maj_num/absolute_max)
                        self.relative_cox[i] = relative


    # 相对密度0.7
    def RelativeDensity_7(self, min_i, maj_i): 
        """少数类={ 
                    离群点密度-2:没有自然邻居,
                    噪声点-3:比例超过0.7,
                    安全点:自然邻居全是少数类样本，
                    边界点:自然邻居有多数类也有少数类,
                    }
        """
        self.relative_cox = [0]*len(self.target)    # 多数类样本的默认密度为0
        for i, num in self.nan.items():   # num为i的自然邻居集合

            if self.target[i] == min_i:     #少数类
                if len(num) == 0 :  # 标记离群点样本 
                    self.relative_cox[i] = -2
                else:
                    absolute_min,min_num,absolute_max,maj_num = 0,0,0,0
                    maj_index = []

                    for j in iter(num): # 遍历自然邻居集合
                        if self.target[j] == min_i:
                            absolute_min += np.sqrt(np.sum(np.square(self.data[i]-self.data[j])))
                            min_num += 1
                        elif self.target[j] == maj_i:
                            absolute_max += np.sqrt(np.sum(np.square(self.data[i]-self.data[j])))
                            maj_num += 1
                            maj_index.append(j)
                    self.nan[i].difference_update(maj_index)    #去掉自然邻居中的多数类样本
                    
                    """
                        如果0.7算成噪声的话，会出现某些少数类样本的自然邻居是空集,
                        这个问题在all_smote_v5.py的generate_samples_Lm()
                        和_smote_variants_v5.py的SMOTE()中已经解决
                    """
                    if min_num == 0 or maj_num >= (min_num + maj_num)*0.8:  # TODO: 比例超过0.7的算噪声样本
                        self.relative_cox[i] = -3 
                    elif maj_num == 0 :         # 安全点: 少数类其自然邻居集合都为少数类
                        relative = min_num/absolute_min
                        self.relative_cox[i] = relative
                    else:           # 边界样本
                        relative = (min_num/absolute_min)/(maj_num/absolute_max)
                        self.relative_cox[i] = relative


def NaN_RD(data_train):
    """
    NData:              原始数据集
    NN.relative_cox:    相对密度权重
    NN.nan: 	        每个样本的自然邻居; [{},{},...,{}]
    """

    NN = Natural_Neighbor()
    NN.data = data_train[:, 1:]  # 初始化数据集
    NN.target = data_train[:, 0]
    NN.algorithm()          # 得到NaN集合

    count = Counter(NN.target)     # 获取少/多数类下标
    c = count.most_common(len(count))
    min_i,maj_i = c[1][0],c[0][0]

    # 计算绝对密度
    NN.RelativeDensity_7(min_i, maj_i)
    NN.relative_cox = np.array(NN.relative_cox)      # 去噪后的密度权重

    # 合并数据特征与标签
    data = np.c_[NN.target, NN.data]

    # 转换自然邻居格式
    NN.nan = np.array([list(v) for v in NN.nan.values()])    

    return data, NN.relative_cox, NN.nan    #原始数据，相对密度，自然邻居
